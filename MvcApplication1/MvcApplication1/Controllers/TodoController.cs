...
namespace MvcApplication1.Controllers
{
    ...
    public class TodoController : ApiController
    {
        ...
        public HttpResponseMessage PutTodoItem(int id, TodoItemDto todoItemDto)
        {
            ...
            // Необходимо отключить, чтобы избежать исключения повторяющегося первичного ключа при вызове SaveChanges
        ...
        public HttpResponseMessage PostTodoItem(TodoItemDto todoItemDto)
        {
            ...
            if (todoList.UserId != User.Identity.Name)
            {
                // Попытка добавить запись, которая не принадлежит пользователю
                return Request.CreateResponse(HttpStatusCode.Unauthorized);
            }

            TodoItem todoItem = todoItemDto.ToEntity();

            // Необходимо отключить, чтобы избежать исключения циклической ссылки при сериализации JSON
            db.Entry(todoList).State = EntityState.Detached;
        ...
        public HttpResponseMessage DeleteTodoItem(int id)
        {
            ...
            {
                // Попытка удалить запись, которая не принадлежит пользователю
                return Request.CreateResponse(HttpStatusCode.Unauthorized);
            }
